...
def nu(vc, alpha, g, T, deltaT, L):                  # Définition du nombre de Nusselt
    ...
    nusselt = 0.6*ray^(1/4)                          # Expression du nombre de Nusselt
    return nusselt

def coeffh(vd, vc, beta, g, alpha, L, H, deltaT, l, Q, long):
    ray = rayleigh(vc, alpha, g, T, deltaT, L)
    h = nu(ray) * l/L                               # Expression de h en fonction du nombre
